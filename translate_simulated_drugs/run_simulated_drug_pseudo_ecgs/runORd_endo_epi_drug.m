...
function runORd_endo_epi_drug(gender, drug_no, effect, IKr_drug_scale, INaL_drug_scale, ICaL_drug_scale, INa_drug_scale, Ito_drug_scale, IK1_drug_scale, IKs_drug_scale)
tic;

p.IKr_drug_scale = IKr_drug_scale;
p.INaL_drug_scale = INaL_drug_scale;
p.ICaL_drug_scale = ICaL_drug_scale;
p.INa_drug_scale = INa_drug_scale;
p.Ito_drug_scale = Ito_drug_scale;
p.IK1_drug_scale = IK1_drug_scale;
p.IKs_drug_scale = IKs_drug_scale;
...
parsave(Vm, cv_est, gendertype, drug_no, effect)

disp(drug_no)

%end
ts_sample = ts(1:5:end,1);
Tname = sprintf('ts_gen%d_Ncell%d_endo%d_epi%d.mat', gendertype, Ncell, Ncell_endo, Ncell_epi); save(Tname, 'ts_sample');
...
%%
    ...
    function parsave(Vm, cv_est, gendertype, drug_no, effect)
    Vm_sample = Vm(:, 1:5:end);
    Vname = sprintf('Vm_gen%d_%s_%d.mat',gendertype, effect, drug_no); save(Vname, 'Vm_sample');
    cv_cable_name = sprintf('cv_cable_gen%d_%s_%d.mat', gendertype, effect, drug_no); save(cv_cable_name, 'cv_est');
