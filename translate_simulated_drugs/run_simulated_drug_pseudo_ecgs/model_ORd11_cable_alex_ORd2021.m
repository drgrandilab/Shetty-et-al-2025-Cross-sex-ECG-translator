...
IKr_drug_scale = p.IKr_drug_scale;
INaL_drug_scale = p.INaL_drug_scale;
ICaL_drug_scale = p.ICaL_drug_scale;
INa_drug_scale = p.INa_drug_scale;
Ito_drug_scale = p.Ito_drug_scale;
IK1_drug_scale = p.IK1_drug_scale;
IKs_drug_scale = p.IKs_drug_scale;
...
INa=INa_drug_scale.*p.f_I(p.iina,:)'.*INa_Multiplier.*GNa.*(v-ENa).*m.^3.*((1.0-fINap).*h.*j+fINap.*hp.*jp);
...
INaL=INaL_drug_scale.*p.f_I(p.iinal,:)'.*INaL_Multiplier.*GNaL.*(v-ENa).*mL.*((1.0-fINaLp).*hL+fINaLp.*hLp);
...
Ito=Ito_drug_scale.*p.f_I(p.iito,:)'.*Ito_Multiplier.*Gto.*(v-EK).*((1.0-fItop).*a.*i+fItop.*ap.*ip);
...
zca=2.0;
PCa=0.0001*ones(Nm,1).*ICaL_Multiplier.*ICaL_drug_scale;
...
IKr=IKr_drug_scale.*p.f_I(p.iikr,:)'.*IKr_Multiplier.*GKr.*sqrt(ko/5.4).*xr.*rkr.*(v-EK);
...
IKs=IKs_drug_scale.*p.f_I(p.iiks,:)'.*IKs_Multiplier.*GKs.*KsCa.*xs1.*xs2.*(v-EKs);
...
IK1=IK1_drug_scale.*p.f_I(p.iik1,:)'.*IK1_Multiplier.*GK1.*sqrt(ko).*rk1.*xk1.*(v-EK);
